...
class _VetProfilePageState extends State<VetProfilePage> {
  ...
  Future<void> _acceptShareRequest(_ShareRequest request) async {
    try {
      ...
      await _database
          .ref(
            "users/${request.ownerUid}/pets/${request.petId}/shared_with/$_uid",
          )
          .set({"email": _currentUser?.email ?? "", "role": request.role});
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Accepted share request for ${request.petName}"),
          ),
        );
      }
    } catch (e) {
  ...
  Future<void> _rejectShareRequest(_ShareRequest request) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Rejected share request for ${request.petName}"),
          ),
        );
      }
    } catch (e) {
                ...
                if (requestData is Map) {
                  requests.add(
                    _ShareRequest(
                      ...
                      petName:
                          requestData['petName']?.toString() ?? 'Unknown Pet',
            ...
            child: StreamBuilder(
              ...
              builder: (context, snapshot) {
                ...
                if (!snapshot.hasData ||
                  ...
                  return const Center(
                    child: Text("No pets shared with you yet."),
                  );
                }
                ...
                data.forEach((ownerUid, ownerData) {
                  if (ownerData is Map && ownerData["pets"] != null) {
                    final pets = Map<dynamic, dynamic>.from(
                      ownerData["pets"] as Map,
                    );
                    pets.forEach((petId, petData) {
                      if (petData is Map && petData["shared_with"] != null) {
                        ...
                        if (sharedWith.containsKey(_uid!)) {
                          ...
                          final species =
                              petData['species']?.toString() ?? 'Unknown';
                        ...
                        builder: (context, constraints) {
                          ...
                          return GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final speciesColor = _speciesColor(
                                pet.displaySpecies,
                              );
                              final speciesLabel = _titleCase(
                                pet.displaySpecies,
                              );
